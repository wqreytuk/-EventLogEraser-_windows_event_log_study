#include<windows.h>
#include<stdio.h>
DWORD _do_it_again(DWORD64 rcx, DWORD edx, DWORD r8d, BYTE* _fixed_data);
int main() {
	BYTE _fixed_data[1024] = {  };
	HANDLE _file_handle = CreateFileA("C:\\Users\\123\\Downloads\\sec.evtx",
		GENERIC_ALL,
		0,
		NULL,
		OPEN_EXISTING,
		FILE_ATTRIBUTE_NORMAL,
		NULL);

	LARGE_INTEGER li;
	li.QuadPart = 0x1000; // Set the offset in bytes
	!SetFilePointerEx(_file_handle, li, NULL, FILE_BEGIN);

	char buffer[0x201] = { 0 };
	DWORD out = 0;
	ReadFile(_file_handle,
		buffer,
		0x200,
		&out,
		NULL
	);

	DWORD64 rcx = reinterpret_cast<DWORD64>(buffer);

	DWORD64 r11 = 0x78;
	DWORD64 r10 = r11 - 1;
	r10 = r10 >> 2;
	DWORD64 r9 = rcx + 1;
	r10 = r10 + 1;

	// 下面是循环体
	DWORD ecx = 0;
	DWORD r8d = 0xffffffff;
	BYTE r8b = 0xff;
	DWORD eax = 0;
	DWORD64  rax = 0;
	DWORD edx = 0;
	DWORD64 rsi = reinterpret_cast<DWORD64>(_fixed_data);
	while (1) {
		ecx = *(reinterpret_cast<BYTE*>(r9 - 1));
		rcx = ecx;
		r9 = r9 + 4;
		eax = r8b;
		rax = eax;
		rcx = rcx ^ rax;
		r8d = r8d >> 8;
		eax = *(reinterpret_cast<BYTE*>(r9 - 4));
		rax = eax;
		edx = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		edx = edx ^ r8d;
		ecx = (BYTE)edx;
		rcx = ecx;
		rcx = rcx ^ rax;
		edx = edx >> 8;
		r8d = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		ecx = *(reinterpret_cast<BYTE*>(r9 - 3));
		rcx = ecx;
		r8d = r8d ^ edx;
		eax = (BYTE)r8d;
		rax = eax;
		rcx = rcx ^ rax;
		r8d = r8d >> 8;
		edx = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		ecx = *(reinterpret_cast<BYTE*>(r9 - 2));
		rcx = ecx;
		edx = edx ^ r8d;
		eax = (BYTE)edx;
		rax = eax;
		rcx = rcx ^ rax;
		edx = edx >> 8;
		r8d = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		r8d = r8d ^ edx;
		r10 = r10 - 1;
		r8b = (BYTE)r8d;
		if (r10 == 0)break;
	}
	rcx = reinterpret_cast<DWORD64>(buffer+0x80);
	
	printf("0x%08x", _do_it_again(rcx, 0x180, r8d, _fixed_data) ^ 0xffffffff);
}

DWORD _do_it_again(DWORD64 rcx2, DWORD edx2, DWORD r8d2, BYTE* _fixed_data) {
	DWORD64 r11 = edx2;
	DWORD64 r10 = r11 - 1;
	r10 = r10 >> 2;
	DWORD64 r9 = rcx2 + 1;
	r10 = r10 + 1;

	// 下面是循环体
	DWORD ecx = 0;
	DWORD r8d = r8d2;
	BYTE r8b = (BYTE)r8d;
	DWORD eax = 0;
	DWORD64  rax = 0;
	DWORD64  rcx = 0;
	DWORD edx = 0;
	DWORD64 rsi = reinterpret_cast<DWORD64>(_fixed_data);
	while (1) {
		ecx = *(reinterpret_cast<BYTE*>(r9 - 1));
		rcx = ecx;
		r9 = r9 + 4;
		eax = r8b;
		rax = eax;
		rcx = rcx ^ rax;
		r8d = r8d >> 8;
		eax = *(reinterpret_cast<BYTE*>(r9 - 4));
		rax = eax;
		edx = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		edx = edx ^ r8d;
		ecx = (BYTE)edx;
		rcx = ecx;
		rcx = rcx ^ rax;
		edx = edx >> 8;
		r8d = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		ecx = *(reinterpret_cast<BYTE*>(r9 - 3));
		rcx = ecx;
		r8d = r8d ^ edx;
		eax = (BYTE)r8d;
		rax = eax;
		rcx = rcx ^ rax;
		r8d = r8d >> 8;
		edx = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		ecx = *(reinterpret_cast<BYTE*>(r9 - 2));
		rcx = ecx;
		edx = edx ^ r8d;
		eax = (BYTE)edx;
		rax = eax;
		rcx = rcx ^ rax;
		edx = edx >> 8;
		r8d = *(reinterpret_cast<DWORD*>(rsi + rcx * 4));
		r8d = r8d ^ edx;
		r10 = r10 - 1;
		r8b = (BYTE)r8d;
		if (r10 == 0)break;
	}
	return r8d;
}
